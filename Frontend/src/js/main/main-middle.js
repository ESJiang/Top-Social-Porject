...
export const setMiddleHighlineImg = () =>
    document
        ...
        .forEach((value, index) => (value.src = `//ik.imagekit.io/fangweij/highline/highline_${index + 1}.webp`));
export const setMiddleProfileImg = () =>
    document
        .querySelectorAll(".main-middle-card.popup-trigger>.profile-photo>img")
        .forEach((value, index) => (value.src = `//ik.imagekit.io/fangweij/profile/profile_${index + 1}.webp`));
...
export const handleCardSearch = function () {
    ...
    search_value === ""
        ...
        : (currentCards = originalCards.filter(item => item.profile.name.toLowerCase().includes(search_value)));
